...
int	rwnx_plat_userconfig_load_8800dc(struct rwnx_hw *rwnx_hw){
    ...
    } else if (rwnx_hw->usbdev->udev->descriptor.idProduct == 0x0126 && rwnx_hw->usbdev->udev->descriptor.idVendor==0x2c4e) {
	if (rwnx_hw->usbdev->country_code[0] == cc[0] && rwnx_hw->usbdev->country_code[1] == cc[1]) {
...
void rwnx_get_countrycode(struct rwnx_hw *rwnx_hw)
{
    const u32 mem_addr = 0x40038030;
    u32 read_mem_addr = 0x83fd124;
    ...
    flash_size = rd_mem_addr_cfm.memdata & 0xff;
    if (flash_size == 0x16) {
	read_mem_addr = 0x83fd124;
    } else if (flash_size == 0x15) {
	read_mem_addr = 0x81fd124;
    }
    memset(&rd_mem_addr_cfm, 0, sizeof(rd_mem_addr_cfm));
    ret = rwnx_send_dbg_mem_read_req(rwnx_hw, read_mem_addr, &rd_mem_addr_cfm);
    if (ret) {
        AICWFDBG(LOGERROR, "%x rd fail: %d\n", read_mem_addr, ret);
	return;
    }
    cc = rd_mem_addr_cfm.memdata & 0xffff;
    if (cc != 0xffff) {
	memcpy(&rwnx_hw->usbdev->country_code[0], &cc, sizeof(cc));
    }
    printk("cc is %x\n", cc);
...
void system_config_8800dc(struct rwnx_hw *rwnx_hw){
    ...
    if (chip_mcu_id) {
        ...
        if (rwnx_hw->usbdev->udev->descriptor.idProduct == 0x0126 && rwnx_hw->usbdev->udev->descriptor.idVendor==0x2c4e) {
	    rwnx_get_countrycode(rwnx_hw);
	}
    }
